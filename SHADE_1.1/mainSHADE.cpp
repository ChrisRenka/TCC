...
int main(int argc, char **argv){
    ...
    for(int r=0;r<runs;r++){
        ...
        if(!alloc){
            sprintf(nameFitFile, "tests/SHADE_f%d_D%d_fitness.txt", func, pop->d);
            sprintf(nameDivFile, "tests/SHADE_f%d_D%d_diversity.txt", func, pop->d);
            sprintf(nameOutput, "tests/SHADE_f%d_D%d_output.txt", func, pop->d);
    
            fitFile = fopen(nameFitFile, "w+");
            divFile = fopen(nameDivFile, "w+");
            output = fopen(nameOutput, "w+");
            
            fprintf(output, "SHADE, function number %d, %d dimensions\n", func, pop->d);
