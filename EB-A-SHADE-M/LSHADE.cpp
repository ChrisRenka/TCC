...
void generateTrialPop(Population *pop, int totalGens, int currentGen){
    ...
    for(int i=0;i<pop->np;i++){
        ...
        //Non-Uniform Michalewicz Mutation        
        partsNumber = 5;
        partsSize = totalGens/partsNumber;
        mutIndexes.reserve(pop->d * pop->mut_p);
        selMut = dist_cross(mt);
        if(selMut < pop->mut_sel){
            for(int j=0;j<(pop->d * pop->mut_p);j++){
                do{
                    dimMut = dist_swap(mt);
                }while(find(mutIndexes.begin(), mutIndexes.end(), dimMut) != mutIndexes.end());
                mutIndexes.push_back(dimMut);
            }
            
            for(unsigned int j=0;j<mutIndexes.size();j++){
                indexMicha = indexMichalewicz(currentGen, partsSize);
                rMut = dist_cross(mt);
                tempMut = 1 - pow(rMut, pow(1 - (indexMicha/(double)partsSize), pop->mut_b));
                eMut = dist_cross(mt);
                if(eMut < 0.5){
                    pop->trial[i]->data[mutIndexes[j]] = pop->trial[i]->data[mutIndexes[j]] + (pop->ui - pop->trial[i]->data[mutIndexes[j]])*tempMut;
                }
                else{
                    pop->trial[i]->data[mutIndexes[j]] = pop->trial[i]->data[mutIndexes[j]] - (pop->trial[i]->data[mutIndexes[j]] - pop->li)*tempMut;
                }
            }
...
int indexMichalewicz(int gen, int partSize){
    int index;
    int currentPart;
    
    index = gen%partSize;                      
    currentPart = gen/partSize;                  
    if(gen <= currentPart*partSize + partSize/2){
        index = partSize - (2*index/4);
    }
    else{
        index = partSize*3/4 + (index - partSize/2)*2/4;
    }
...
void parametersInput(Population *pop, FILE *arq){
    int np, npi, npf, d, aval, h, mut_b;
    double li, ui, c, p, rArc, mut_sel, mut_p;
    int temp;
    ...
    temp = fscanf(arq, "mut_sel: %lf\n", &mut_sel);
    temp = fscanf(arq, "mut_p: %lf\n", &mut_p);
    temp = fscanf(arq, "mut_b: %d\n", &mut_b);
